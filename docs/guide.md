...
The result contains one segment per input circle. `len(cap_coverage)` is the
...
For a long, thin region, give `cover_sweep()` the sampled left and right edges.
Each pair of consecutive samples becomes one quadrilateral:
...
>>> point_cells = px.cell_at(
...     unit_vector(point_lon, point_lat), resolution=4
... )
```
...
>>> lonlat = to_lonlat(point_centers)
```
