...
| `count_caps_per_cell()` | Retain as a measured fused accelerator that avoids materializing cap-cell hits. |
| `count_coverage_per_cell()` / `sum_coverage_per_cell()` | Geometry-neutral native reductions over `Coverage`; admitted for dense and positional selected-cell outputs. |
| `occupancy_runs()` | Lossless cell-major ordinal windows; replaces the mixed, lossy canonical `summarize_occupancy()` result. |
